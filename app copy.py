...
# 10) 이전 대화 내용을 화면에 다시 적는다.
# 사용자와 챗봇의 대화를 채팅 창처럼 보여준다.
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        st.markdown(message["content"])

# 11) 사용자 입력창을 만든다.
# 사용자가 질문을 적고 엔터를 누르면 그 값이 question 변수에 들어온다.
question = st.chat_input("문서에 대해 질문해 보세요.")

# 12) 사용자가 질문을 입력하면 실행되는 부분
if question:
    # 사용자가 보낸 질문을 기록에 저장한다.
